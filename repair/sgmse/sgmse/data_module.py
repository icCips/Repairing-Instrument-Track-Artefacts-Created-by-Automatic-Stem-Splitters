...
class Specs(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.normalize == "noisy":
            if y.numel() > 0:  # Check if tensor y is not empty
                normfac = y.abs().max()
            else:
                normfac = 1.0  # Default value if tensor y is empty
        elif self.normalize == "clean":
            if x.numel() > 0:  # Check if tensor x is not empty
                normfac = x.abs().max()
            else:
                normfac = 1.0  # Default value if tensor x is empty
        elif self.normalize == "not":
            normfac = 1.0
