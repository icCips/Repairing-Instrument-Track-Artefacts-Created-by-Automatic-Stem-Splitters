    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0 and self.num_eval_files != 0:
            ...
            for (clean_file, noisy_file) in zip(clean_files, noisy_files):
                ...
                #MODIFIED (unused)
                x_16k_contiguous = np.ascontiguousarray(x_16k, dtype=np.float32)
                x_hat_16k_contiguous = np.ascontiguousarray(x_hat_16k, dtype=np.float32)

                #pesq_sum += pesq(16000, x, x_hat, 'wb') 
