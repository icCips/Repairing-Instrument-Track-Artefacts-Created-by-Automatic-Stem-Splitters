...
def energy_ratios(s_hat, s, n):
    ...
    si_sdr = 10 * np.log10(np.linalg.norm(s_target)**2 / (np.linalg.norm(e_noise + e_art)**2 + 1e-8))
    si_sir = 10 * np.log10(np.linalg.norm(s_target)**2 / (np.linalg.norm(e_noise)**2 + 1e-8))
    si_sar = 10 * np.log10(np.linalg.norm(s_target)**2 / (np.linalg.norm(e_art)**2 + 1e-8))

    # SI-SNR
    s_ = s - np.mean(s)
    s_hat_ = s_hat - np.mean(s_hat)
    alpha = np.dot(s_hat_, s_) / (np.linalg.norm(s_)**2 + 1e-8)
    s_target_snr = alpha * s_
    e_noise_snr = s_hat_ - s_target_snr
    si_snr = 10 * np.log10(np.sum(s_target_snr**2) / (np.sum(e_noise_snr**2) + 1e-8))

    return si_sdr, si_sir, si_sar, si_snr
