...
def compute_model_output(model, inputs):
    ...
    if model.separate:
        for inst in model.instruments:
            ...
            all_outputs[inst] = output[inst].detach().clone().cpu()
    else:
        all_outputs = model(inputs).detach().cpu()
...
def predict_song(args, audio_path, model):
    ...
    # In case we had to pad the mixture at the end, or we have a few samples too many due to inconsistent down- and upsampling, remove those samples from source prediction now
