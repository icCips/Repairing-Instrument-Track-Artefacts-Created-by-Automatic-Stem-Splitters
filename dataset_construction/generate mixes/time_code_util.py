...
def cut_audio(audio_file, sr, start_sample, end_sample):
    ...
    if start_sample >= len(audio_file):

        return np.zeros((segment_length, 2))
    
    if end_sample > len(audio_file):
        end_sample = len(audio_file)

    segment = audio_file[start_sample:end_sample]
    
    if len(segment) < segment_length:
        padding_length = segment_length - len(segment)
        segment = np.pad(segment, ((0, padding_length), (0, 0)), mode='constant')
...
def generate_sample_indices(length_in_samples, sample_rate):
    ...
    # Ensure minimum excerpt length is less than the total length
