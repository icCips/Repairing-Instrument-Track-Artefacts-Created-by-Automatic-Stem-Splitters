...
def get_max_db(data):
    # Find the peak (maximum absolute value) in the audio data
...
def is_white_noise(audio_file, start_time_ms = 10000, duration_ms = 3000, volume_threshold=-20.0):
    ...
    # Extract a slice of the audio
    end_time_ms = start_time_ms + duration_ms
    audio_slice = audio[start_time_ms:end_time_ms]
    
    # Apply high-pass filter at 7k Hz
    audio_filtered = high_pass_filter(audio_slice, 7000)
    
    # Measure the loudness of the filtered audio
    loudness = audio_filtered.dBFS  # Decibels relative to full scale
...
'''def normalize_audio(data, samplerate, target_dBFS=-10.0):
    ...
    # Load the temporary file using pydub
    audio = AudioSegment.from_file(temp_filename)
    
    # Normalize the audio to the target dBFS level
    normalized_audio = audio.apply_gain(target_dBFS - audio.max_dBFS)
    
    # Save the normalized audio to a BytesIO object
    with io.BytesIO() as output:
        normalized_audio.export(output, format='wav')
        output.seek(0)
        # Load the WAV data from BytesIO into a NumPy array
        normalized_data, _ = sf.read(output)
