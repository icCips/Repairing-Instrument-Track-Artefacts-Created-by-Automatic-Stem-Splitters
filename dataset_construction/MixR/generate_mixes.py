import MixR.process_track as pt
...
def sum_audio_files(stems, level):
    ...
    if max_amplitude > 1.0:
        summed_audio /= max_amplitude
    
    normalised_audio = pt.normalize_audio(summed_audio, SR, level)
    
    return normalised_audio

def generate_mixes(version, dataset_path):
    
    print(f"Generating {version} mixes...")
    
    base_dir = os.path.join(dataset_path, "raw_unzipped")
    base_dest = os.path.join(dataset_path, "generated_mixes")
