...
def compute_loss(model, inputs, targets, criterion, compute_grad=False):
    ...
    '''
    device = inputs.device  # Ensure inputs are on the correct device
    ...
    if model.separate:
        ...
        for inst in model.instruments:
            ...
            loss = criterion(output[inst], targets[inst].to(device))
        ...
        for inst in all_outputs.keys():
            loss += criterion(all_outputs[inst], targets[inst].to(device))
